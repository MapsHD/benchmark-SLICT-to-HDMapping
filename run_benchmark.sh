...
# run_benchmark.sh - Reusable benchmark runner for SLICT (ROS1)
set -e
...
echo "=========================================="
echo "Running SLICT benchmark (ROS1)"
...
IMAGE="${SLICT_IMAGE:-ghcr.io/mapshd/slict2hdmapping:latest}"
...
docker run --rm -t \
    ...
    bash -lc "
      ...
      # SLICT uses colcon, so source install/setup.bash
      if [ -f /test_ws/install/setup.bash ]; then source /test_ws/install/setup.bash; fi
      ...
      roslaunch ${SLICT_LAUNCH} >/tmp/slict.log 2>&1 &
      LAUNCH_PID=\$!
      ...
      rosrun slict-to-hdmapping listener \"\${REC_BAG}\" \"\${OUT_DIR}/hdmapping\"
    "
